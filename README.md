...
5. The timer can take some time to reset. If a user goes too quickly through the questions, as one would do whilst testing, then the code does not have enough time to load between questions and it can hang. This will not be a problem in a normal scenario in which a user would take their time to read through, list to the sound clip and then answer the question.
...
Please <a href="https://github.com/Claire-Potter/Disney-Star/tree/master/read-me-content/testing/browser-testing" target="_blank">click here</a> to view browser testing screenshots.
