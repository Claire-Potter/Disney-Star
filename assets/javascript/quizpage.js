 ...
 */
 ...
 function getNewQuestion() {
   ...
   for (let i = 0; i < optionLen; i++) {
     ...
     //disable the submitButton so that the user cannot move passed the question until answered. Code added by me.
   ...
   /** 
    * @function myTimer() is utilised below to set a 45 second count per question. This will update the timer div with the countdown. 
    * I got the code for the timer from https://stackoverflow.com/questions/44314897/javascript-timer-for-a-quiz and this has been adapted for the site by me. The if else scenarios have been added by me
    * and indicate what to do if the user answers in time and gets the answer correct, answers in time but gets the answer wrong, or does not
    * answer in time.
    */
   ...
   function myTimer() {
     ...
     sec--; {
       // if the user answers correctly and in time, the message will display that they beat the timer. The style of the timer dive will be updated accordingly.
       if (document.getElementById(0) == document.querySelector('.option.correct.already-answered') || document.getElementById(1) == document.querySelector('.option.correct.already-answered') || document.getElementById(2) == document.querySelector('.option.correct.already-answered') || document.getElementById(3) == document.querySelector('.option.correct.already-answered')) {
         clearInterval(time);
         document.getElementById('timer').innerHTML = 'You beat the timer!';
         document.getElementById('timer').classList.add('in-time');
         // if the user answers incorrectly but in time, the message will display that they answered in time but they got it wrong. The style of the timer dive will be updated accordingly.
       } else if (document.getElementById(0) == document.querySelector('.option.wrong.already-answered') || document.getElementById(1) == document.querySelector('.option.wrong.already-answered') || document.getElementById(2) == document.querySelector('.option.wrong.already-answered') || document.getElementById(3) == document.querySelector('.option.wrong.already-answered')) {
         clearInterval(time);
         document.getElementById('timer').innerHTML = 'You answered in time, but you got it wrong';
         document.getElementById('timer').classList.add('wrong-time');
         // if the user does not answer in time, an alert message will display that they did not answer in time and the quizOver function will be called.
       } else if (sec == -1) {
         clearInterval(time);
 ...
 function unclickableOptions() {
   ...
   for (let i = 0; i < optionLen; i++) {
     ...
     //when the options class already-answered has been added, re-enable the submitButton to access the next question.
 ...
 function nextQuestion() {
   if (questionCounter === questionLimit) {
     ...
     getNewQuestion();
     // remove the timer styling so that default style displays for the next question
 ...
 //Add an event listener for when the user selects to return to the home page to trigger to scroll to te top of the screen. Code added by me.
